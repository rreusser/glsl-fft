...
Perform the setup work required to use the FFT kernel in the fragment shader, `fft.glsl`. Input arguments are:
...
- `resolution` (Number): size of the transform, equal to the width or height. Currently must be a power of two and the width must equal the height.
...
### `vec4 fft(sampler2D src, float resolution, float subtransformSize, bool horizontal, bool forward, bool normalize)`
